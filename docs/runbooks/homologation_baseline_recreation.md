...
6. Executar **`homologation:provision`** no release novo, com `DEPLOYMENT_ENV=homologation`, contas fornecidas em arquivo privado e sandbox de pagamentos configurado, seguindo o procedimento abaixo. O comando cria administrador de negócio (`admin`, não `root`), parceiro e consumidor, memberships e conteúdo demonstrativo completo. Não importar seed de desenvolvimento nem dumps financeiros parciais. Contas `.local` opcionais são criadas posteriormente pelo comando explícito de teste descrito abaixo, sem copiar registros antigos. Não mudar homologação para development. Guardar o recibo de IDs emitido pelo comando; nenhuma senha é impressa.
...
Esse procedimento nominal não modifica a restrição do `development_seeder`. A exceção deliberada para contas de teste usa exclusivamente o comando separado abaixo; não é consequência automática da recriação.

## Contas de teste opcionais

**Opcional, somente para testes em implantação não produtiva.** Depois do provisionamento nominal, o operador pode acrescentar três contas memorizadas pela equipe, inclusive com e-mails `.local`, sem usar o development seeder. O comando separado evita confundir a base nominal com a exceção de teste:

```sh
# Artefato compilado, apontando para o banco de homologação conferido acima
node ace.js homologation:provision-test-accounts --allow-test-accounts --config=/run/private/test-accounts.json
```

No checkout TypeScript, usar `pnpm ace homologation:provision-test-accounts --allow-test-accounts --config=/run/private/test-accounts.json`. Usar a mesma montagem privada somente leitura, UID e imagem já conferidos no procedimento nominal; não executar automaticamente no deploy. Não selecionar outro banco por inferência.

- O sinal **`--allow-test-accounts`** é obrigatório além do nome do comando. Sem ele, exit 1. `DEPLOYMENT_ENV` deve ser exatamente `homologation` ou `development`; `production`, ausência e valor inválido são recusados, inclusive em reexecução. `NODE_ENV=production` continua correto para a imagem compilada de homologação.
- Preparar outro JSON privado com a mesma estrutura (`tenantSlug`, `tenantName`, `accounts.administrator`, `accounts.partner`, `accounts.customer`). Nome e slug devem identificar o tenant nominal existente. Preencher os três e-mails desejados e suas senhas somente nesse arquivo, nunca em argumento, código, fixture, documento ou log. Continuam as proteções de arquivo externo ao artefato, modo `0600`, proprietário correto, sem symlink/hardlink e limite de 16 KiB.
- Somente este modo admite `.local` e senhas legadas de **8–128 caracteres**, sem exigir composição nem senhas distintas entre contas. Os e-mails precisam ser distintos. Não há senha padrão/fallback; todas são fornecidas pelo operador. O modo nominal preserva 20–128 caracteres, composição, senhas distintas e recusa de `.local`.
- É um acréscimo à base: cria administrativa global **root**, membro owner do tenant e da organização; parceira global user, membro do tenant e admin da organização, com capacidades de validar e ler histórico de resgate; consumidora global user, membro do tenant e com nova cortesia na edição gratuita já provisionada. O root tem poder global nessa instância, não apenas no tenant. Não cria/replica fichas, mídia, edição, oferta, compra ou pagamento e não altera contas nominais.
- Exige recibo da baseline nominal, tenant/organização ativos, edição gratuita publicada e em período de uso, com oferta ativa de estabelecimento publicado. Base ausente ou inválida é recusada; executar primeiro o provisionamento nominal, sem estender janelas silenciosamente.
- O recibo tem `created: true`, IDs das três novas contas e do acesso; não contém e-mails ou senhas. O aviso identifica contas de TESTE e o poder global root. Guardar o arquivo no cofre e apagar a cópia temporária; a saída não é canal de recuperação de senha.
- Repetir com as mesmas identidades retorna `created: false` e o recibo original, sob lock do tenant e transação atômica. Não duplica, não gira senha, reativa conta, restaura papéis removidos, renova cortesia/cota nem apaga resgates. Identidade já existente sem recibo próprio é recusada, nunca adotada ou promovida. Se as contas antigas ainda existem, tratar sua identidade explicitamente pela administração; este comando não as sobrescreve.
- Validar autenticação das três contas, carteira do consumidor e apresentação → preview/validação → confirmação → histórico pela parceira. A exceção de identidade não relaxa autenticação, tenant, policies, HTTPS, origem canônica de apresentação ou bloqueios financeiros. A aplicação de homologação precisa da origem HTTPS configurada conforme o runbook de compras.
- **Nunca promover essa base para produção de negócio.** O comando não é um sanitizador de dados existentes: mudar somente a variável de ambiente não apaga contas, não troca senhas nem remove root. Produção deve partir de base/identidades próprias e credenciais não previsíveis; a execução deste comando será recusada mesmo com a flag. Não acionar o seeder de desenvolvimento na homologação.
