...
### Implementação das fotos — 23/09/2026

Até esta data a avaliação guardava só os contadores `photos_count` e `videos_count`, e o primeiro era **declarado pelo cliente**: era possível afirmar quatro fotos sem enviar nenhuma. As fotos passam a existir, em `establishment_review_photos`, e as decisões são estas:

- **Mesmo pipeline do ADR-0014.** `files` e `media_assets`, a mesma sonda de imagem, os mesmos formatos (JPEG, PNG e WebP) e o mesmo limite de 10 MiB. HEIC continua recusado, como a divergência acima registra. Chaves compostas prendem cada foto à avaliação **e** ao estabelecimento do asset, de modo que nenhuma foto aponte para imagem de outro lugar ou de outra operação.
- **Sem moderação própria por foto.** A foto faz parte da avaliação e é pública exatamente quando a avaliação é: sai com denúncia resolvida como `content_hidden`, com banimento do autor e com a retirada do estabelecimento do catálogo. A mídia de estabelecimento e a de conteúdo do parceiro passam por fila prévia porque são material comercial publicado em nome do negócio; a avaliação é conteúdo do Explorador moderado por denúncia, e o texto dela já é publicado assim. Uma fila prévia só para as fotos criaria uma segunda fila humana, que o item 9 do Anexo I não dimensiona — a operação contínua de moderação está fora da contratação — e faria a avaliação aparecer sem as próprias fotos por tempo indeterminado. Para a denúncia ser julgada pelo que foi visto, **a fila de denúncias mostra as fotos** da avaliação denunciada.
- **Os metadados da imagem são removidos antes de tudo.** Foto de celular carrega EXIF com a posição GPS, o aparelho e o horário, além de XMP e comentários. A foto de um estabelecimento mostra um endereço que já é público; a de um Explorador pode revelar onde ele mora. O arquivo guardado e servido perde esses metadados antes de a sonda calcular o checksum, e só a **orientação** é mantida, num bloco EXIF mínimo, para que fotos em retrato não apareçam deitadas. Se a estrutura do arquivo não puder ser percorrida, ele é recusado, nunca guardado como chegou. A remoção vale para as fotos de avaliação; estender à mídia de estabelecimento é decisão separada.
- **Limite da política, janela de edição.** A quantidade vem de `max_photos` do tenant, nunca de constante, e `max_photos = 0` desliga o envio. Anexar respeita a janela de edição, mas não o intervalo mínimo entre edições: esse intervalo existe para frear reescritas, e aplicá-lo aqui impediria anexar fotos logo depois de escrever.
