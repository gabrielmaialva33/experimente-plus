...
export default class extends BaseSchema {
  protected tableName = 'establishment_events'
  ...
  async up() {
    this.schema.createTable(this.tableName, (table) => {
      ...
      table.unique(['id', 'tenant_id'], 'establishment_events_id_tenant_unique')
      table
        .foreign('tenant_id', 'establishment_events_tenant_foreign')
        .references('id')
        .inTable('tenants')
        .onDelete('RESTRICT')
      table
        .foreign(
          ...
          'establishment_events_establishment_tenant_foreign'
        )
        .references(['id', 'tenant_id'])
        .inTable('establishments')
        .onDelete('RESTRICT')
      table
        .foreign(['created_by', 'tenant_id'], 'establishment_events_creator_tenant_foreign')
      ...
      table
        .foreign('archived_by', 'establishment_events_archiver_foreign')
        .references('id')
        .inTable('users')
        .onDelete('RESTRICT')
      ...
      table.index(
        ...
        'establishment_events_establishment_status_index'
      )
      table.index(['tenant_id', 'status', 'created_at'], 'establishment_events_moderation_index')
      table.check(
        "status IN ('draft', 'pending_review', 'published', 'archived')",
        [],
        'establishment_events_status_check'
      )
      table.check(
        "published_snapshot IS NULL OR jsonb_typeof(published_snapshot) = 'object'",
        [],
        'establishment_events_snapshot_object_check'
      )
      table.check(
        '(published_snapshot IS NULL) = (published_at IS NULL)',
        [],
        'establishment_events_publication_pair_check'
      )
      table.check(
        "status <> 'published' OR published_snapshot IS NOT NULL",
        [],
        'establishment_events_published_snapshot_check'
      )
      table.check(
        ...
        [],
        'establishment_events_archive_check'
      )
      table.check('ends_at > starts_at', [], 'establishment_events_window_check')
    })

    this.schema.raw(`
      CREATE FUNCTION prevent_establishment_events_deletion()
      RETURNS trigger LANGUAGE plpgsql AS $$
      BEGIN
        RAISE EXCEPTION 'Partner content must be archived, never deleted' USING ERRCODE = '23514';
      END;
      $$;
      CREATE TRIGGER establishment_events_prevent_delete
      BEFORE DELETE ON establishment_events
      FOR EACH ROW EXECUTE FUNCTION prevent_establishment_events_deletion();
    `)
  }

  async down() {
    this.schema.dropTable(this.tableName)
    this.schema.raw('DROP FUNCTION IF EXISTS prevent_establishment_events_deletion()')
  }
}
